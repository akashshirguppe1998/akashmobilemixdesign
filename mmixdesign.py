...
if selected=="DESIGN":
	...
	if st.button("GET MIX PROPORTION"):
		...
		if l==0 and m==0 and n==0:
			...
			if m==0 and n==0:
				...
				st.write("CHEMICAL ADMIXTURE : **%.2f Kg/m^3** ." % P1)
			elif n==0:
				st.write("MINERAL ADMIXTURE 1 : **%.2f Kg/m^3** ." % l2)
				st.write("MINERAL ADMIXTURE 2 : **%.2f Kg/m^3** ." % m2)
				st.write("CHEMICAL ADMIXTURE : **%.2f Kg/m^3** ." % P1)	
			else:
				st.write("MINERAL ADMIXTURE 1 : **%.2f Kg/m^3** ." % l2)
				st.write("MINERAL ADMIXTURE 2 : **%.2f Kg/m^3** ." % m2)
				st.write("CHEMICAL ADMIXTURE : **%.2f Kg/m^3** ." % n2 )
				st.write("CHEMICAL ADMIXTURE : **%.2f Kg/m^3** ." % P1)
				
		st.write("WATER CEMENT RATIO : **%.4f** ." % w_cmp)
		st.write("******")
	...
	with st.expander("**CLICK HERE FOR DETAILED OUTPUT**"):
		...
		if FrMoCo1!=0 or FrMoCo2!=0:
			...
			if l==0 and m==0 and n==0:
				st.write("9.1) CEMENT : **%.2f Kg/m^3**" % Cement1)	
			else:
				if m==0 and n==0:
					st.write("9.1.a) CEMENT (OPC) : **%.2f Kg/m^2**" % Cement1)
					st.write("9.1.b) 1st MINERAL ADMIXTURE CONTENT : **%.2f Kg/m^3**" % l2)
				elif n==0:
					st.write("9.1.a) CEMENT (OPC) : **%.2f Kg/m^2**" % Cement1)
					st.write("9.1.b) 1st MINERAL ADMIXTURE CONTENT :** %.2f Kg/m^3**" % l2)
					st.write("9.1.c) 2nd MINERAL ADMIXTURE CONTENT : **%.2f Kg/m^3**" % m2)
				
				else:
					st.write("9.1.a) CEMENT : **%.2f Kg/m^2**" % Cement1)
					st.write("9.1.b) 1st MINERAL ADMIXTURE CONTENT : **%.2f Kg/m^3**" % l2)
					st.write("9.1.c) 2nd MINERAL ADMIXTURE CONTENT : **%.2f Kg/m^3**" % m2)
					st.write("9.1.d) 3rd MINERAL ADMIXTURE CONTENT : **%.2f Kg/m^3**" % n2)		
			st.write("9.2)Water : **%.4f Kg/m^3**" %water22)
			if FrMoCo1<=0:
				st.write("9.3)Coarse aggregate (dry) : **%.2f Kg/m^3**" % MCA1)
			elif FrMoCo1>0:
				st.write("9.3)Coarse aggregate (wet) : **%.2f Kg/m^3**" % MCA1)
			if FrMoCo2<=0:	
				st.write("9.4)Fine aggregate(dry): **%.2f Kg/m^3**" % MFA1)
			elif FrMoCo2>0:
				st.write("9.4)Fine aggregate (wet) : **%.2f Kg/m^3**" % MFA1)
			st.write("9.5)Chemical admixture : **%.2f Kg/m^3**" % P1)
